...
export function CadastroForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!registerResult.success) throw new Error(registerResult.message)

      // 2️⃣ Fazer login automaticamente para pegar perfil e respondeu
      const loginResult = await login(email, password)
      if (!loginResult.success) throw new Error(loginResult.message)

      // 3️⃣ Redirecionar baseado em respondeu/perfil
      if (!loginResult.respondeu) {
        router.push(`/formulario?usuario=${loginResult.user?.id}`)
      } else if (loginResult.perfil) {
        router.push(`/perfil/${loginResult.perfil.toLowerCase()}`)
      } else {
        router.push(`/formulario?usuario=${loginResult.user?.id}`)
      }
    } catch (err) {
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="grid gap-2 relative">
              ...
              <button
                ...
                className="absolute right-2 top-2/3 -translate-y-1/2 h-full flex items-center justify-center px-1 text-muted-foreground hover:text-foreground"
              >
